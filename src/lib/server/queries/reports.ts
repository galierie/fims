...
import { faculty, facultycourse, course, semester, facultysemester, facultyadminposition, rank, facultyeducationalattainment, adminposition, facultycommmembership, facultyresearch, facultyadminwork, facultyrank, facultyhomeaddress, facultyemail, fieldofinterest, facultyfieldofinterest, facultycontactnumber, research, office } from '../db/schema';
...
export async function getFacultyServiceRecordReport(facultyid: number, fromAcadYear: number, fromSemNum: number, toAcadYear: number, toSemNum: number) {
    const existingFacultySemesterSq = db
        ...
        .where(and(
            ...
            or(
                ...
                and(
                    eq(semester.academicyear, toAcadYear),
                    lte(semester.semester, toSemNum),
                ),
                and(
    ...
    const profileQuery = db
        ...
        .from(faculty)
        .innerJoin(existingFacultySemesterSq, eq(faculty.facultyid, existingFacultySemesterSq.facultyid))
        .innerJoin(semester, eq(existingFacultySemesterSq.acadsemesterid, semester.acadsemesterid))
        .leftJoin(facultyrank, eq(existingFacultySemesterSq.currentrankid, facultyrank.facultyrankid))
        .leftJoin(rank, eq(facultyrank.rankid, rank.rankid))
        .leftJoin(facultyeducationalattainment, eq(existingFacultySemesterSq.currenthighesteducationalattainmentid, facultyeducationalattainment.facultyeducationalattainmentid))
    ...
    const originalTenureQuery = db
        ...
        .where(and(eq(faculty.facultyid, facultyid), eq(facultyrank.appointmentstatus, 'Permanent')));
    ...
    const adminPositionsQuery = db
        ...
        .innerJoin(adminposition, eq(facultyadminposition.adminpositionid, adminposition.adminpositionid))
        .innerJoin(office, eq(facultyadminposition.officeid, office.officeid))
        .innerJoin(existingFacultySemesterSq, eq(facultyadminposition.facultysemesterid, existingFacultySemesterSq.facultysemesterid))
        .groupBy(facultyadminposition.facultyadminpositionid, adminposition.name, office.name)
        .orderBy(desc(facultyadminposition.enddate), desc(facultyadminposition.startdate), desc(facultyadminposition.facultyadminpositionid));
    ...
    const fieldsOfInterestQuery = db
        ...
        .innerJoin(fieldofinterest, eq(facultyfieldofinterest.fieldofinterestid, fieldofinterest.fieldofinterestid))
    ...
    const currentTeachingLoadQuery = db
        ...
        .leftJoin(facultycourse, eq(existingFacultySemesterSq.facultysemesterid, facultycourse.facultysemesterid))
    ...
    const currentAdministrativeLoadQuery = db
        ...
        .from(existingFacultySemesterSq)
        .leftJoin(facultyadminposition, eq(existingFacultySemesterSq.facultysemesterid, facultyadminposition.facultysemesterid))
        .leftJoin(adminposition, eq(facultyadminposition.adminpositionid, adminposition.adminpositionid))
        .leftJoin(facultycommmembership, eq(existingFacultySemesterSq.facultysemesterid, facultycommmembership.facultysemesterid))
        .leftJoin(facultyadminwork, eq(existingFacultySemesterSq.facultysemesterid, facultyadminwork.facultysemesterid))
        .groupBy(existingFacultySemesterSq.acadsemesterid);
    ...
    const currentResearchLoadQuery = db
        ...
        .from(existingFacultySemesterSq)
        .leftJoin(facultyresearch, eq(existingFacultySemesterSq.facultysemesterid, facultyresearch.facultysemesterid))
    ...
    const [profile, originalTenure, adminPositions, fieldsOfInterest, semestralRecords, currentTeachingLoad, currentAdministrativeLoad, currentResearchLoad] = await Promise.all([
        profileQuery,
        originalTenureQuery,
        adminPositionsQuery,
    ...
    return { profile, originalTenure, adminPositions, fieldsOfInterest, semestralRecords, currentTeachingLoad, currentAdministrativeLoad, currentResearchLoad }
}
